...
import { Avatar, Drawer, List, Badge, Card, Comment } from "antd";
...
const CommentList = ({ comments }) => (
  <InfiniteScroll dataLength={comments.length} scrollableTarget="scrollableDiv">
    <List
      dataSource={comments}
...
class ChatDetails extends React.Component {
   ...
   */
  chatDetails = () => {
    if (this.props.chooseUser.messageTypes !== 2) {
      return;
    }
    axiosGet(Params.GROUP_USER_URL + this.props.chooseUser.toUser).then(
      (response) => {
          ...
          <Card title={this.props.chooseUser.toUsername} size="larg">
            ...
            >
              {this.props.messageList.length > 0 && (
                <CommentList comments={this.props.messageList} />
              )}
            </div>
          </Card>
        </Badge.Ribbon>
